...
public:
    void UpdateData(uiAmsHumidityInfo *info) { m_ams_id = info->ams_id; UpdateData(info->humidity_display_idx, info->humidity_percent, info->left_dry_time, info->current_temperature); };
...
private:
    void UpdateData(int humidiy_level, int humidity_percent, int left_dry_time, float current_temperature);
    void UpdateContents();

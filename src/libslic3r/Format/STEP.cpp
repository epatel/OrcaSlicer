...
bool StepPreProcessor::isGBK(const std::string str) {
    ...
    while (i < str.length()) {
        if (static_cast<unsigned char>(str[i]) <= 0x7f) {
            ...
            if (static_cast<unsigned char>(str[i]) >= 0x81 &&
                static_cast<unsigned char>(str[i]) <= 0xfe &&
                static_cast<unsigned char>(str[i + 1]) >= 0x40 &&
                static_cast<unsigned char>(str[i + 1]) <= 0xfe &&
                static_cast<unsigned char>(str[i + 1]) != 0xf7) {
                i += 2;
                continue;
            }
            else {
